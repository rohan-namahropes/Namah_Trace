...
const seedBatches = [
  {
    id: "NL-2026-08-20",
    notes: "Production run for 16mm braided rope.",
    created: "2026-08-20T08:42:00",
    status: "In Progress",
    currentStage: 3,
    stages: [
      {
        ...defaultStages[0],
        status: "Completed",
        performedBy: "Production operator",
        completedAt: "2026-08-20T09:18:00",
        notes: "Yarn cleared for production.",
        measurements: [
          { key: "BS", value: "25 kN" },
          { key: "Elongation", value: "4%" },
        ],
      },
      {
        ...defaultStages[1],
        status: "Completed",
        performedBy: "Arjun S.",
        completedAt: "2026-08-20T11:26:00",
        notes: "Twist consistency checked across sample.",
        measurements: [{ key: "TPM", value: "12" }],
      },
      {
        ...defaultStages[2],
        status: "In Progress",
        performedBy: "Priya K.",
        startedAt: "2026-08-20T13:10:00",
        notes: "Colour matching in progress.",
        measurements: [],
      },
  ...
  {
    id: "BATCH-001",
    notes: "Black polypropylene rope sample.",
    created: "2026-08-19T10:16:00",
    status: "Completed",
    currentStage: 6,
    stages: defaultStages.map((stage, index) => ({
      ...stage,
      status: "Completed",
      performedBy: index % 2 ? "Arjun S." : "Production operator",
      completedAt: `2026-08-${19 - Math.min(index, 3)}T${10 + index}:20:00`,
      notes:
        index === 0
          ? "All initial samples within expected range."
          : "Stage completed and recorded.",
      measurements: index === 0 ? [{ key: "BS", value: "27 kN" }] : [],
    })),
  },
  {
    id: "XYZ-23",
    notes: "Awaiting incoming material confirmation.",
    created: "2026-08-18T15:03:00",
    status: "On Hold",
    currentStage: 1,
    stages: defaultStages.map((stage, index) => ({
      ...stage,
      status: index === 0 ? "On Hold" : "Pending",
      measurements: [],
    })),
  },
];
...
function App() {
  ...
  if (!session)
    return (
      <Login onLogin={setSession} error={loginError} setError={setLoginError} />
    );
  ...
  const selectedStage = selectedBatch?.stages.find(
  ...
  return (
    <div className="app-shell">
      <Sidebar
        active={selectedBatch ? "Batches" : "Overview"}
  ...
  return (
    <aside className={`sidebar ${mobileOpen ? "open" : ""}`}>
      <div className="side-top">
        ...
        <button className="icon-button side-close" onClick={onClose}>
          <X size={19} />
        </button>
      </div>
    ...
    <section className="page dashboard-page">
      ...
      <div className="stat-grid">
        ...
        <Stat
          ...
          value="01"
          detail="Since 01 Aug 2026"
          icon={<Check size={19} />}
        />
        <Stat
          label="On hold"
          value={batches
            .filter((batch) => batch.status === "On Hold")
            .length.toString()
            .padStart(2, "0")}
      ...
      <div className="section-heading">
        <div>
          <h2>All batches</h2>
          <p>{batches.length} batch records in your workspace</p>
        </div>
      ...
      <div className="batch-table">
        ...
        {filtered.length === 0 && (
          <div className="empty-state">No batches match “{search}”.</div>
        )}
      </div>
    </section>
  );
...
function BatchRow({ batch, onClick }) {
  ...
  return (
    <button className="batch-row" onClick={onClick}>
      <div className="batch-name">
        <span className="batch-symbol">{batch.id.slice(0, 1)}</span>
        <div>
      ...
      <div className="stage-cell">
        <span
          ...
          style={{
            background:
              batch.stages[Math.max(0, batch.currentStage - 1)]?.color,
          }}
        ></span>
        {batch.currentStage >= 6
          ? "Packaging"
          : batch.stages[batch.currentStage - 1]?.name || "Not started"}
      </div>
      ...
      <div className="date-cell">
        ...
        <small>{formatTime(batch.created)}</small>
      </div>
  ...
  return (
    <section className="page batch-page">
      ...
      <div className="batch-title-row">
        <div>
          <p className="eyebrow">BATCH RECORD</p>
          <h1>{batch.id}</h1>
          <p className="subheading">
      ...
      <div className="batch-overview">
        <div>
          ...
          <p className="progress-caption">
            {
              batch.stages.filter((stage) => stage.status === "Completed")
                .length
            }{" "}
            of {batch.stages.length} stages completed
          </p>
        </div>
        <div className="batch-note">
          ...
          <div>
            <small>BATCH NOTES</small>
            <p>{batch.notes || "No notes added to this batch."}</p>
          </div>
        </div>
      </div>
      <div className="pipeline-section">
        ...
        <div className="pipeline">
          {batch.stages.map((stage, index) => (
            <button
              className={`pipeline-stage stage-${stage.status.toLowerCase().replace(" ", "-")}`}
              key={stage.id}
      ...
      <div className="history-section">
        ...
        <div className="timeline">
          {history.map((entry, index) => (
            <div className="timeline-item" key={`${entry.date}-${index}`}>
              ...
              <div className="timeline-content">
                ...
                <p>
                  {entry.action} by <b>{entry.person}</b>
          ...
          {history.length === 0 && (
            <div className="empty-state">No history has been recorded yet.</div>
          )}
        </div>
      </div>
    </section>
...
function CreateBatch({ onClose, onCreate }) {
  ...
  return (
    <div className="modal-backdrop">
      <form className="modal create-modal" onSubmit={submit}>
        ...
        <p className="modal-intro">
          Give this production run a unique identity. You can add to its record
        ...
        <label>
          ...
          <div className="upload-box">
            ...
            <span>
              ...
              <small>Photos or documents, up to 10MB</small>
            </span>
            <input type="file" />
          </div>
        </label>
...
function StageDetail({ stage, batch, displayName, onClose, onSave }) {
  ...
  const addMeasurement = () => {
    if (newKey && newValue) {
      setMeasurements([...measurements, { key: newKey, value: newValue }]);
      setNewKey("");
      setNewValue("");
    }
  };
  return (
    <div className="modal-backdrop">
      <div className="modal stage-modal">
        <div className="modal-head">
          <div className="stage-modal-title">
            <span className="stage-number">
              {stage.status === "Completed" ? <Check size={15} /> : "03"}
            </span>
            <div>
              <p className="eyebrow">STAGE RECORD</p>
              <h2>{stage.name}</h2>
            </div>
          </div>
        ...
        <div className="stage-status-tabs">
          {["Pending", "In Progress", "Completed", "On Hold"].map((option) => (
            <button
              key={option}
        ...
        <div className="form-grid">
          ...
          <label>
            ...
            <div className="input-with-icon">
              <Clock3 size={16} />
              <input value={formatDate(new Date())} readOnly />
            </div>
          </label>
        </div>
        <label>
          ...
          <textarea
            ...
            onChange={(event) => setNotes(event.target.value)}
            rows="3"
            placeholder="Record what happened in this stage..."
          />
        </label>
        <div className="measurements">
          <div className="subsection-title">
            <div>
              <h3>Measurements</h3>
              <p>Flexible key-value fields for this stage.</p>
            </div>
        ...
        <label>
          ...
          <div className="upload-box compact">
            ...
            <span>
              ...
              <small>Files will be stored with this stage record</small>
            </span>
            <input
              type="file"
              multiple
        ...
        <div className="modal-actions">
          ...
          >
            Save stage record <Check size={16} />
          </button>
        </div>
      </div>
    </div>
